...
1. **Read context.** Load `GOAL.md` for the project overview and `NOTES.md` for techstack, gotchas, and preferences.
...
4. **Move to In Progress.** Update `TASKS.md` by moving the task from Backlog to In Progress. **This is the first required update.**

   Format: `- [ ] Task description`

5. **Work on the task.** Implement the requirement. Monitor your context usage as you work.

6. **Handle partial completion.** If you exceed approximately 150k tokens of context and cannot finish the task, add a progress note directly in `TASKS.md` next to the in-progress item, then output `[STOP LOOP]`:
...
10. **Update notes (if needed).** If you made significant changes, update `NOTES.md`:
...
- **Always update TASKS.md twice:** once when moving from Backlog to In Progress (step 4), and again when moving to Completed (step 8).
