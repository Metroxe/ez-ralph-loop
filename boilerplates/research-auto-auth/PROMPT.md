...
3. **Follow up on existing leads first.** Pursue any unexplored leads from the "Leads to Follow Up" section of `NOTES.md` before running new searches. **Remove each lead from the list once you have followed up on it** — move the results to the appropriate section (Searched Sources, Dead Ends, etc.). **Do not revisit sources already listed in `NOTES.md`** unless you have reason to believe they have new information.
